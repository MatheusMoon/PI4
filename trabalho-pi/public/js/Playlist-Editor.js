...
async function savePlaylist() {
    ...
    // Verifica se todos os itens possuem ID (importante!)
    const validItems = playlistItems.every(item => item.id);

    if (!validItems) {
        alert('Todos os itens da playlist precisam ter um ID válido!');
        return;
    }
    ...
    const playlistData = {
        ...
        unicode: playlistItems.map(item => ({
            type: item.type,
            id: item.id,  // Garantindo que o ID seja atribuído corretamente
            duration: item.time
        })),
...
async function fetchPlaylistData() {
    ...
    if (window.location.pathname.startsWith('/edit') && playlistId) {
        try {
            const response = await fetch(`/api/edit/${playlistId}`);
            const data = await response.json();

            if (response.ok) {
                // Passa os dados da playlist (que inclui os itens) para loadPlaylistItems
                loadPlaylistItems(data.playlist); 
            } else {
                console.error('Erro ao buscar dados da playlist:', data.error);
            }
        } catch (error) {
...
async function loadPlaylistItems() {
    ...
    if (playlistId && window.location.pathname.startsWith('/edit')) {
        try {
            // Chama a API para buscar os dados da playlist
            const response = await fetch(`/api/edit/${playlistId}`);
            const data = await response.json();
            ...
            if (response.ok) {
                ...
                // Carregar os itens da playlist dentro dos wrappers
                const itemsContainer = document.getElementById('items-container'); // Supondo que você tenha um container para os itens
                
                for (let item of data.playlist.items) {
                    const slideWrapper = createSlideWrapper('item-class', null, item.duration);  // Cria o slide wrapper com a duração

                    // Verificar o tipo do item e adicionar o elemento correspondente
                    if (item.type === 'image' && item.mediaData) {
                        // Buscar a imagem
                        const mediaElement = await loadImage(item.mediaData.id);
                        slideWrapper.appendChild(mediaElement);
                    } else if (item.type === 'video' && item.mediaData) {
                        // Buscar o vídeo
                        const mediaElement = await loadVideo(item.mediaData.id);
                        slideWrapper.appendChild(mediaElement);
                    } else if (item.type === 'text' && item.mediaData) {
                        // Buscar o texto
                        const textElement = await loadText(item.mediaData.id);
                        slideWrapper.appendChild(textElement);
                    }

                    // Adiciona o slide wrapper ao container
                    itemsContainer.appendChild(slideWrapper);
                }
            } else {
                console.error('Erro ao buscar dados da playlist:', data.error);
            }
...
async function loadImage(id) {
    ...
    return createMediaElement('image', imageUrl, false);  // Cria o elemento de imagem
}
...
async function loadVideo(id) {
    ...
    return createMediaElement('video', videoUrl, true);  // Cria o elemento de vídeo
}
...
async function loadText(id) {
    ...
    if (!response.ok) {
        throw new Error('Erro ao carregar o texto');
    }
    const data = await response.json();
    const textElement = createTextElement(data.content);

    // Se houver uma imagem associada ao texto, define como fundo
    if (data.backgroundImageUrl) {
        textElement.style.backgroundImage = `url('${data.backgroundImageUrl}')`;
    }

    return textElement;
}

// Função para criar o elemento de texto a ser adicionado ao slide
function createTextElement(content) {
    const textElement = document.createElement('div');
    textElement.className = 'text-item';  // Classe para estilizar o texto
    textElement.textContent = content;  // Define o conteúdo do texto
    return textElement;
}


window.onload = fetchPlaylistData;
